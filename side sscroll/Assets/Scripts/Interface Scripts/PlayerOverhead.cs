...
public class PlayerOverhead : MonoBehaviour
{
    ...
    void Update ()
    {
        if (connectedPlayer != null && overhead_display != null)
        {
            ...
            if (life > 5)
            {
                (overhead_display.transform.Find("InterfaceOverheadLabel").GetComponent<Image>()).enabled = false;
                (overhead_display.transform.Find("InterfaceArrow").GetComponent<Image>()).enabled = false;
            }
            //update our position
