...
public class AiTrainerManager
{
    ...
    public void Start ()
    {
        ...
        //load our files
        for (index=0; index<maxGenome; index++)
        {
    ...
    {	
        ...
        {			
            ...
            double mutationChance = UnityEngine.Random.Range((float)mutationPercentLower, (float)mutationPercentUpper);
			
            float mutateRate = (float)mutationAmount * 10000;
    ...
    {
        double procced = UnityEngine.Random.Range(0, 1.0F);
		
        if (mutationChance > procced)
        {
            //mutate this node
            double amount = UnityEngine.Random.Range(-mutateRate, mutateRate);
